...
def build_cartesia_tts():
    ...
    try:
        if hasattr(CartesiaTTSService, "Settings"):
            ...
            logger.info(f"Cartesia: Settings OK")
    ...
    try:
        ...
        logger.error(f"Cartesia fallita: {e}")
        raise
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async for raw_message in websocket.iter_text():
        ...
        if msg.get("event") == "start":
            ...
            logger.info(f"Chiamata avviata | stream={stream_sid}")
            break
    ...
    # FIX SALUTO: aspetta pipeline pronta prima di mandare il saluto
    ...
    async def on_connected(t, c):
        ...
        # Piccolo delay per assicurarsi che la pipeline sia pronta
        import asyncio
